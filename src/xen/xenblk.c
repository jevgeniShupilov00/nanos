...
struct xenblk_dev {
    ...
    vector reqs;    /* xenblk_ring_req */
    struct list pending, done;  /* xenblk_req */
    struct list free;  /* xenblk_ring_req */
...
static xenblk_ring_req xenblk_get_req(xenblk_dev xbd)
{
    ...
    if (req != INVALID_ADDRESS) {
        req->id = vector_length(xbd->reqs);
        vector_push(xbd->reqs, req);
    } else {
        req = 0;
    }
    spin_unlock_irq(&xbd->lock, irqflags);
...
static void xenblk_service_pending(xenblk_dev xbd)
{
    ...
    while (prod < prod_end) {
        ...
        xenblk_ring_req rreq = xenblk_get_req(xbd);
        if (!rreq) {
...
static void xenblk_service_ring(xenblk_dev xbd)
{
    ...
    do {
        ...
        while (cons < prod) {
            ...
            xenblk_ring_req rreq = vector_get(xbd->reqs, resp->id);
            assert(rreq);
            for (u8 segment = 0; segment < rreq->segments; segment++)
                xen_revoke_page_access(rreq->grefs[segment]);
            list_insert_before(list_begin(&xbd->free), &rreq->l);
                        ...
                        void *, buf, range, blocks, status_handler, sh)
{
    ...
    xenblk_debug("[%d] %s %R", xbd->dev.if_id, write ? "write" : "read",
            blocks);
    xenblk_req req = allocate(xbd->h, sizeof(*req));
    if (req == INVALID_ADDRESS) {
                        ...
                        xenblk_dev, xbd)
{
    ...
    while ((l = list_get_next(&xbd->done))) {
        ...
        apply(req->sh, req->s);
        deallocate(xbd->h, req, sizeof(*req));
                 ...
                 int, id, buffer, frontend, tuple, meta)
{
    ...
    xbd->reqs = allocate_vector(h, XENBLK_RING_SIZE);
    if (xbd->reqs == INVALID_ADDRESS) {
